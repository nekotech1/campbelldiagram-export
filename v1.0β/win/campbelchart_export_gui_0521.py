...
def main():
    ...
    frame1_content = [
        ...
        [sg.InputText(size=(55,1), k='-FILE_IN-'),sg.FileBrowse(button_text='参照', initial_folder=os.path.expanduser("~"))],
        [sg.Radio("入力フォルダ選択（選択したフォルダ中のファイルをすべてグラフ化）", group_id=1, k="-RADIO_2-")],
        [sg.InputText(size=(55,1), k='-FOLDER_IN-'),sg.FolderBrowse(button_text='参照', initial_folder=os.path.expanduser("~"))],
        [sg.Text("出力フォルダ選択")],
        [sg.InputText(size=(55,1), k='-FOLDER_OUT-'),sg.FolderBrowse(button_text='参照', initial_folder=os.path.expanduser("~"))],
        ]
        # [sg.Text("設定ファイル選択")],
    ...
    layout_left = [[sg.Frame('入出力選択', frame1_content, font=font_1, s=(480,220))],
        [sg.Button("グラフの表示(更新)", border_width=3, size=(15,2), k="-DRAW-"), sg.Button("グラフの処理・保存", border_width=3, size=(15,2), k="-SAVE-")],
        [sg.Frame("描画設定", frame2_content, font=font_1, s=(480,580))],
        [sg.Button("描画設定の読み込み", border_width=3, size=(15,2), k="-OPEN_INI-"), sg.Button("描画設定の保存", border_width=3, size=(15,2), k="-SAVE_INI-"), sg.Button("DEBUG", k="-DEBUG-")]]
    ...
    layout_right = [[sg.Frame("キャンベル線図描画結果", frame2_content, font=font_1, s=(980,910))]]
